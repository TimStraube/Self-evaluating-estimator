...
\begin{tikzpicture}[node distance=2cm, auto]
  ...
  \node (memory) [above=-1cm of agent, xshift=9cm, minimum width=6cm, minimum height=2cm, rounded corners]  {
      ...
      $ [O_0, O_1, ..., O_{C-1}] $ \\
      $ [r_1, r_2, ..., r_C] $\\
      $ r_{c, t + 1} = r_{c, t} + p_r r_{\Delta, c} $ \\
      $ O_{c, t + 1} = \sigma(A_O)(O_{c, t} + \sigma{(A_{G})} O_{U, t + 1}) $ 
      \end{tabular}
  };
  ...
  \node (evaluator) [rectangle, below=-1.4cm of agent, xshift=5.6cm, rounded corners] {
    \begin{tabular}{c}  Evaluation \\ $r_{\Delta} - \frac{1}{C}\sum{ p_r r_c}$ \end{tabular}
  };
  \node (calculator) [rectangle, below=-1.6cm of agent, xshift=11.6cm, rounded corners] {
    \begin{tabular}{c} 
      Wahrnehmungsdissonanz \\ 
      $O_c(A_G) = O_c - O_{U, t}$ \\
      $O(A_G) = \frac{1}{C} \sum_c A_{G, c} O_c$ \\ 
      $\Delta O = O(A_{G})-O_{U, t}$ \\ 
      $ r_{\Delta, c} = -\Delta O_{c} $ \\
      $ r_{\Delta} = \sum p_r r_{\Delta, c} $
    \end{tabular}
  };
